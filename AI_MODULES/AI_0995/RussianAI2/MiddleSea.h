...
void ProcessMiddleSeaAI(){
	...
	if(NKrep){
		...
		if(GetUnits(&AKADEMIA)>0){
			...
			TryUnit(&KAZ17,5,100,100); //my addition
		};
		if(GetUnits(&STABLE)) TryUnit(&CENTER,5,100,100);
		...
		TryUnit(&KAZ18,1,100,20);
		TryUnit(&KAZ18,5,100,5);
...
//			if(GetMoney(WoodID)>100000)AI_Torg(WoodID,GoldID,90000);
	...
	if(GetPeaceTimeLeft()>=30&&GetUnits(&KAZ18)>=5)TryUnit(&KAZ17,15,95,100);

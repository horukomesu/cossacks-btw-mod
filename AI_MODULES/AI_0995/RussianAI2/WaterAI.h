...
void ProcessWaterAI(){	
	...
	if(!GetAIRegister(AIR_StartCoal+AiNat)&&GetReadyUnits(&RINOK)){
		if(STRES==0) AI_Torg(CoalID,FoodID,1000);
		if(STRES==1) AI_Torg(CoalID,FoodID,2500);
		if(STRES==2) AI_Torg(CoalID,FoodID,4000);
		...
		//if(UpgIsRun(&MELN_U)){
			if(!FieldExist()){
				...
				if(Food>700){
					...
					if(GetPeaceTimeLeft()<=20&&GetMoney(FoodID)>800) TryUnit(&INFANTRY,100,100,100);
				};				
				...
				if(InfIsStrong){
					if(Food>200){
						if(BuildPs)TryUnit(&PEASANT,1500,100,100);
						if(Food>300 && GetPeaceTimeLeft()<=20&&GetMoney(FoodID)>800)TryUnit(&INFANTRY,NeedInf,100,30);
					};
				}else{ //useless! (InfIsStrong==true at the start of function!!!
					if(Food>200){
						TryUnit(&PEASANT,135,100,100);
						if(GetPeaceTimeLeft()<=20&&GetMoney(FoodID)>800) TryUnit(&INFANTRY,50,100,30);
						if(GetPeaceTimeLeft()<=20&&GetMoney(FoodID)>800) TryUnit(&INFANTRY,150,5,15);
					};
				};
	...
	if(GetPeaceTimeLeft()<=20){
		...
		if(!NBLD){
			if(GetUnits(&STABLE)) TryUnit(&CENTER,4,95,5);
		};
		...
		if(!NBLD && GetUnits(&STABLE)) TryUnit(&DIPCENTER,1,99,10);
...
//??this section disabled mine upgrades
	...
	if(FastHorse){
		...
		if(GetUnits(&STABLE)) TryUnit(&KAZ17,2,95,90);
	};
	if(UpgIsRun(&FLOT)){
		TryUpgrade(&B_SHIELD1,95,90);
		TryUpgrade(&B_SHIELD2,95,90);
	};
	if(GetReadyUnits(&RINOK)&&STRES<3){
		...
		if(UpgIsDone(&B_SHIELD1)&&UpgIsDone(&B_SHIELD2)){
			...
			if(GetMoney(StoneID)>50000&&!UpgIsRun(&FASTFISH)) AI_Torg(StoneID,WoodID,45000);
			if(GetMoney(StoneID)>110000) AI_Torg(StoneID,GoldID,90000);
		};
/*		if(GetMoney(WoodID)>150000){
		...
		if(GetMoney(FoodID)>500000) AI_Torg(FoodID,WoodID,400000);
	};
	TryUpgrade(&EN_FREG18,99,100);
	if(GetUnits(&STABLE)) TryUnit(&KAZ17,5,90,90);
	if(GetUnits(&KAZ18)) TryUpgrade(&MELN_U1,95,100);
...
//Economy
	if(GetReadyUnits(&RINOK)){
		...
		if(GetMoney(GoldID)>220000&&GetExtraction(GOLD)>200){
			if(GetMoney(IronID)<100000){

...
void UpgradeUnit(){
	...
	if(UpgIsDone(&DRAGUN18_UA5) && UpgIsDone(&GREN18_UA5)){
...
//---added--------------------------------------
//		if(GetPeaceTimeLeft()<=GetMaxPeaceTime()/2+GetMaxPeaceTime()/5){
		if(GetPeaceTimeLeft()<=20){
			if(GetUnits(&INFANTRY)<140){
				TryUnit(&INFANTRY, 800, 20, 100);
			}else{
				if(GetUnits(&KAZ17)>=10) TryUnit(&INFANTRY, 1200, 20, 100);
				if(GetUnits(&KAZ17)<10) TryUnit(&INFANTRY, 800, 20, 100);
...
//		if(GetPeaceTimeLeft()<=GetMaxPeaceTime()/2) TryUnit(&GREN18, 2000, 20, 100);
...
void Market(){
	...
	if(GetStartRes()==SR_MILLIONS){
		...
		if(GetUnits(&CENTER)>5&&GetMoney(StoneID)>250000){
			AI_Torg(GoldID,StoneID,5000);
			AI_Torg(StoneID,GoldID,100000);
		};

		if(GetUnits(&CENTER)>5&&GetUnits(&STABLE)>4&&GetMoney(WoodID)>250000){
			AI_Torg(GoldID,WoodID,5000);
	...
	if(GetReadyUnits(&RINOK)){
		...
		*/
		...
		// Stone to Gold
		if(GetMoney(StoneID)>250000) SellingIron();

		// Wood to Gold
		if(GetMoney(WoodID)>350000&&GetUnits(&STABLE)>=4) SellingWood();

		if(GetMoney(WoodID)>200000&&GetUnits(&STABLE)<4) SellingWood();
...
void BuildingsMillions(){
	...
	if(GetUnits(&KAZ18)>6){
		TryUnit(&KAZ17,GetUnits(&KAZ17)+1,100,100);
//		TryUnit(&STABLE,9,100,100);
	};
};
...
void NavalScript(){
	//Units
	TryUnit(&PAROM, 1, 100, 100);
	TryUnit(&YAHTA, 3, 100, 100);
	TryUnit(&GALLEY, 30, 100, 100);
	...
	if(UpgIsRun(&FLOT)){
		...
		TryUnit(&PORT,4,100,100);
		TryUnit(&PORT,7,100,100);
	};
...
*/
	//Distribution
	if(UpgIsDone(&AKASTN2) && !UpgIsRun(&FLOT))
		SetPOrder(6,4,9, "быстрая постройка кораблей");

	//строить галеры
	if(UpgIsRun(&FLOT))
		SetPOrder(6,9,3, "строить галеры");
};

void ProcurementScript(){
	...
	if(UpgIsRun(&AKAWOOD)){
		...
		if(UpgIsRun(&AKASTN2)){
			TryUnit(&CENTER,4,100,100);
			TryUnit(&CENTER,5,100,100);
			TryUnit(&MELN,3,100,100);			
			TryUnit(&SKLAD,3,100,100);
			TryUnit(&SKLAD,4,100,100);
		};
	};
	...
	TryUpgrade(&MELN_U1, 98, 100);
	if(UpgIsDone(&XVIII) && 
	!(UpgIsRun(&MELN_U1) || GetMoney(FoodID) > (GetUpgradeCost(AiNat, &MELN_U1, FoodID)*11)/10)){
		SetPOrder(9,2,2, "второй на мельнице");
	};
	//Distribution
	/*if(!UpgIsRun(&XVIII)){
		//int payment = GetUpgradeCost(AiNat, &XVIII, FoodID);
		if(GetMoney(FoodID) < GetUpgradeCost(AiNat, &XVIII, FoodID)*1.1)
			SetPDistribution(9,2,9);
		else
			SetPDistribution(9,6,6);
	};*/
	if(GetReadyUnits(&AKADEMIA)>0){
		TryUpgrade(&AKASTN2, 92, 100);
		if(!(UpgIsRun(&AKASTN2))){ 								
			if(GetMoney(FoodID) < (GetUpgradeCost(AiNat, &AKASTN2, FoodID)*13)/10)
				SetPDistribution(9,9,1);
		...
		TryUpgrade(&AKAFOOD1, 100, 100);
		TryUpgrade(&KUZ_FIELD, 100, 100);
		if(!(UpgIsRun(&AKAFOOD1) && UpgIsRun(&KUZ_FIELD))) 
			SetPDistribution(9,4,1);
	};

	TryUnit(&AKADEMIA,1,100,100);
	if(GetReadyUnits(&KAZ17)>0 && 
			GetUnits(&AKADEMIA)==0)
		SetPOrder(6,4,4, "академия");

	TryUpgrade(&MELN_U, 50, 100);
	if(GetReadyUnits(&MELN)>0 && !UpgIsRun(&MELN_U))
		SetPOrder(9,1,1, "первый на мельнице");
...
void ExtrMillions(){
	...
	else if(GetUnits(&CENTER)<6||GetUnits(&STABLE)<5) SetPDistribution(9,5,3);
